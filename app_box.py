...
def recognize_face(new_embedding, embedding_database, threshold=1.0):
    ...
    print(f"Min distance: {min_distance}, Threshold: {threshold}")  # Debugging line
...
# Function to display image with detected or recognized name and bounding box
def display_image_with_name_and_box(img, name, facial_area):
    ...
    img_display.thumbnail((min(top.winfo_screenwidth() // 2, 400), min(top.winfo_screenheight() // 2, 400)))
...
def add_new_face(image_path, name):
    ...
    if isinstance(faces, dict):
        for key in faces:
            ...
            messagebox.showinfo("Success", f"Face enrolled as '{name}'")

            # Display the image with name and bounding box
            display_image_with_name_and_box(img, name, facial_area)
            return
    else:
    ...
    if isinstance(faces, dict):
        for key in faces:
            ...
            if name == "Unknown":
                ...
                messagebox.showinfo("Result", f"Recognized as {name}")
